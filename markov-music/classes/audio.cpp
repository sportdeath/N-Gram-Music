...
void Audio::detectOnsets(std::vector<int> * onsets) {
    ...
    double * odfValues = new double [sampleArrays[0] . size()/hopSize];
    
    odf -> process((int) sampleArrays[0] . size(), &sampleArrays[0][0], (int) sampleArrays[0] . size()/hopSize, odfValues);
    
    int peakWindow = ceil(0.02*sampleRate/hopSize); //audio transient in music around 20ms
    
    for (int i = peakWindow; i < sampleArrays[0] . size()/hopSize - peakWindow; i++) {
        bool isOnset = true;
