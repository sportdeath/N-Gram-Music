...
void RealNote::initializeSampleArray(Song * song, long startFrame, long endFrame) {
    std::vector<double> * songVector = song -> getSampleArray();
    
    std::vector<double> sampleVector(songVector-> begin() + startFrame,songVector -> begin() + endFrame);
    
    this -> setSampleArray(sampleVector);
}
...
////relative power of this note to the one in front of it.
//double RealNote::getRelativePower() {
//    Note * next = this -> getNextNote();
//    if (next -> isEndNote()) {
//        return 10000;
//    }
//    
//    RealNote * nextNote;
//    nextNote = dynamic_cast<RealNote *>(next);
//    
//    return (this -> getPower())/(nextNote -> getPower());
//}
